...
DATA_ACCESS progress maps Received, Verified, Processing, Response ready, and Completed. DATA_DELETION maps Received, Verified, Processing, and Completed. Both use the same compact horizontal track: completed stages use checkmarks, the current stage uses a bordered dot, and upcoming stages retain their step numbers. Waiting requests show an interruption treatment. Rejected and cancelled requests preserve stages reached before closure without marking the workflow completed.
...
Response states distinguish no file, ready for delivery, failed delivery, and delivered successfully. A response file is optional: operators may complete a request without one, while any available public files remain private and are delivered through secure access. DATA_DELETION completion requires an explicit processed-work confirmation, permits one optional internal completion note, notifies the requester, and reaches Completed only after successful delivery. The UI avoids storage terminology and attachment identifiers. Internal notes never expose visibility controls or actor IDs. Activity history is collapsed by default and translates audit events into human-readable labels without rendering raw event payloads, provider identifiers, hashes, or tokens.
