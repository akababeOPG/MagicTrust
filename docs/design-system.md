...
Response states distinguish no file, ready for delivery, failed delivery, and delivered successfully. A response file is optional: operators may complete a request without one, while any available public files remain private and are delivered through secure access. DATA_DELETION, DIRECT_PROCESSING, and CONVERSATIONAL_PROCESSING completion require an explicit processed-work confirmation, permit one optional internal completion note, notify the requester, and reach Completed only after successful delivery. Conversational processing keeps Complete request primary, progressively discloses Wait for requester as the secondary action, and presents Resume processing as the sole primary action while waiting. Waiting requires a public message and successful delivery before the workflow advances. DIRECT_PROCESSING uses generic workflow copy in the operator workspace; request-intent wording is limited to the centralized consumer presentation layer. The UI avoids storage terminology and attachment identifiers. Internal notes never expose visibility controls or actor IDs. Activity history is collapsed by default and translates audit events into human-readable labels without rendering raw event payloads, provider identifiers, hashes, or tokens.

## Consumer Secure Access

The secure consumer request page uses a centered, narrow layout without the admin shell. Its hierarchy is branding and secure-access context, the current request outcome, available response files, request details, then non-empty public updates. For a conversational request waiting on the requester, public messages move directly below the outcome so the required next step is clear. For completed requests with public response files, the response download is the primary action and appears before request metadata.
